          ...
          <>
            <div className="grid gap-4 sm:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
              {paginatedProjects.map((project) => (
                <article key={project._id} className="card-panel overflow-hidden flex flex-col">
                  <div className="relative overflow-hidden">
                    <img src={project.images?.[0]} alt={project.title} className="h-36 sm:h-44 lg:h-48 w-full object-cover" />
                    {project.images && project.images.length > 1 && (
                      <div className="absolute top-2 right-2 bg-black/60 backdrop-blur-sm text-white px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-full text-xs sm:text-sm font-semibold flex items-center gap-1">
                  ...
                  <div className="space-y-2 sm:space-y-3 flex-grow px-4 sm:px-6 py-4 sm:py-6 flex flex-col">
                    <div className="flex flex-wrap items-start justify-between gap-2">
                      <div className="flex-grow min-w-0">
                        ...
                        <h2 className="mt-0.5 sm:mt-1 text-base sm:text-lg font-semibold text-slate-950 line-clamp-2">{project.title}</h2>
                      </div>
                      <div className="flex-shrink-0">
                        <StatusBadge status={project.status} />
                      </div>
                    </div>
                    <p className="text-xs font-semibold uppercase tracking-[0.15em] text-amber-700">{project.location}</p>
                    <div>
                      <p className="text-xs sm:text-sm leading-5 sm:leading-6 text-slate-600 line-clamp-3">
                        {project.description}
                      </p>
                      {project.description && project.description.length > 150 && (
                        <button
                          ...
                          className="mt-2 text-xs font-semibold text-amber-700 hover:text-amber-800 transition"
                        >
                          View More
                        </button>
                      )}
                    ...
                    {project.completionDate ? (
                      <p className="text-xs text-slate-500 mt-auto">Completion: {formatDate(project.completionDate)}</p>
                    ) : null}
                  </div>
                </article>
              ))}
            ...
            {totalPages > 1 && (
              <div className="mt-6 sm:mt-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 rounded-2xl border border-slate-200 px-4 sm:px-6 py-4">
                ...
                <div className="flex gap-2 justify-center sm:justify-start">
                  <button
                    ...
                    className="rounded-full border border-slate-200 p-2 sm:p-2.5 text-slate-600 disabled:opacity-50 transition hover:bg-slate-50"
                    title="Previous page"
                  >
                    <ChevronLeft size={18} />
                  </button>
