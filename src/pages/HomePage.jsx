    ...
    <div className="space-y-16 sm:space-y-20 pb-6 sm:pb-8 pt-4 sm:pt-6">
      <section className="site-container">
        <div className="grid gap-8 grid-cols-1 lg:grid-cols-[1.1fr_0.9fr] lg:items-stretch">
          <div className="card-panel grid-overlay overflow-hidden px-4 sm:px-6 py-8 sm:py-12 md:px-10 md:py-14">
            <span className="fade-in-up inline-flex w-fit rounded-full border border-white/80 bg-white/80 px-3 py-1 text-xs font-semibold uppercase tracking-[0.24em] text-amber-700">
              Civil Infrastructure Excellence
            </span>
            <h1 className="fade-in-up stagger-1 mt-4 sm:mt-6 max-w-3xl text-2xl sm:text-3xl md:text-5xl lg:text-6xl font-semibold tracking-tight text-slate-950">
              We build roads, structures, and utility networks that keep communities moving.
            </h1>
            <p className="fade-in-up stagger-2 mt-4 sm:mt-6 max-w-2xl text-sm sm:text-base md:text-lg leading-6 sm:leading-7 md:leading-8 text-slate-600">
            ...
            <div className="fade-in-up stagger-3 mt-6 sm:mt-8 flex flex-wrap gap-3 sm:gap-4">
              <Link
                to="/projects"
                className="inline-flex items-center gap-2 rounded-full bg-slate-950 px-4 sm:px-5 py-2.5 sm:py-3 text-xs sm:text-sm font-semibold text-white transition hover:bg-slate-800"
              >
                Explore Projects
                <ArrowRight size={16} />
              </Link>
              <Link
                to="/contact"
                className="rounded-full border border-slate-200 bg-white px-4 sm:px-5 py-2.5 sm:py-3 text-xs sm:text-sm font-semibold text-slate-700 transition hover:border-slate-300"
              >
            ...
            <div className="mt-8 sm:mt-12 grid gap-4 grid-cols-1 sm:grid-cols-3">
              {[
                ...
                <div key={item.label} className="rounded-3xl border border-white/80 bg-white/80 p-3 sm:p-4">
                  <p className="text-xl sm:text-2xl font-semibold text-slate-950">{item.value}</p>
                  <p className="mt-1 sm:mt-2 text-xs sm:text-sm text-slate-600">{item.label}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="grid gap-5">
            <div className="card-panel overflow-hidden bg-gradient-to-br from-slate-700 via-slate-800 to-slate-900 p-6 sm:p-8 text-white shadow-lg hover:shadow-xl transition-shadow duration-300 dark:from-slate-800 dark:via-slate-800 dark:to-slate-900 dark:text-white">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-xs uppercase tracking-[0.24em] font-semibold text-slate-300 dark:text-slate-400">Why Clients Choose Us</p>
                  <h2 className="mt-3 text-lg sm:text-2xl font-semibold text-white">Structured delivery from mobilization to handover</h2>
                </div>
                <div className="rounded-full bg-slate-600/30 p-2 sm:p-3 backdrop-blur-sm dark:bg-slate-700/50 transition-transform duration-300 hover:scale-110 flex-shrink-0">
                  <HardHat className="text-slate-200 dark:text-slate-300" size={28} />
                </div>
              </div>
              <div className="mt-6 sm:mt-8 space-y-3 sm:space-y-4">
                {[
                  ...
                  <div key={point} className="flex items-start gap-3 group">
                    <CheckCircle2 className="mt-0.5 sm:mt-1 text-slate-300 dark:text-slate-400 flex-shrink-0 group-hover:text-white dark:group-hover:text-slate-200 transition-colors" size={18} />
                    <p className="text-xs sm:text-sm leading-6 sm:leading-7 text-slate-200 dark:text-slate-300 group-hover:text-white dark:group-hover:text-white transition-colors">{point}</p>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section className="site-container">
        <div className="mb-6 sm:mb-8 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
          ...
          />
          <Link to="/projects" className="text-xs sm:text-sm font-semibold text-amber-700 transition hover:text-amber-800 whitespace-nowrap">
            See full portfolio
          </Link>
        </div>
        {loading ? (
              ...
              <article key={project._id} className="card-panel overflow-hidden">
                ...
                <div className="space-y-3 sm:space-y-4 px-4 sm:px-6 py-4 sm:py-6">
                  <div className="flex items-center justify-between gap-4">
                    <p className="text-xs sm:text-sm font-medium text-slate-500">{project.location}</p>
                    <StatusBadge status={project.status} />
                  </div>
                  <h3 className="text-lg sm:text-2xl font-semibold text-slate-900">{project.title}</h3>
                  <p className="text-xs sm:text-sm leading-6 sm:leading-7 text-slate-600">{truncateText(project.description, 125)}</p>
                </div>
              </article>
            ))}
          </div>
